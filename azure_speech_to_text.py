...
class SpeechToTextManager:
    ...
    def __init__(self):
        ...
        try:
            self.azure_speechconfig = speechsdk.SpeechConfig(subscription=os.getenv("AZURE_TTS_KEY_2"), region=os.getenv("AZURE_TTS_REGION"))
        except TypeError:
    ...
    def speechtotext_from_mic(self):
        ...
        if speech_recognition_result.reason == speechsdk.ResultReason.RecognizedSpeech:
            ...
            print("\nSpeech Recognition canceled: {}".format(cancellation_details.error_details))
            print("BananaZam")
    ...
    # def speechtotext_from_mic_continuous(self, stop_key='5', max_retries=3):
    #     """
    #     Continuously converts speech from the microphone to text until the stop key is pressed or retries are exhausted.
    #     """
    #     retries = 0
    ...
    #             """Callback for when speech is successfully recognized."""
    #             print('RECOGNIZED: {}'.format(evt))
    #
    #         def stop_cb(evt: speechsdk.SessionEventArgs):
    #             """Callback for when the session stops."""
    #             print('CLOSING speech recognition on {}'.format(evt))
    #             nonlocal done
    #             done = True
    #
    #         def handle_final_result(evt):
    #             """Handles the final result of the recognition."""
    #             all_results.append(evt.result.text)
    #
    #         # Connect callbacks
    ...
    #         print(f"line 157")
    #         result_future.get()  # Ensures the recognizer is initialized
    #         print(f"line 159")
    ...
    #
    #         # After exiting the loop, always attempt to stop recognition
    #         self.azure_speechrecognizer.stop_continuous_recognition_async()
    #         print(f'stopped recognition of voice 164')
    #
    #         elapsed_time = time.time() - start_time
    #         if elapsed_time < 1 or not all_results:
    #             print("Failed too quickly or no results, retrying...")
    #             retries += 1
    #             continue
    #         else:
    #             break
    ...
    def speechtotext_from_mic_continuous(self, stop_key='5'):
        ...
        last_recognizing_text = ""

        def recognizing_cb(evt: speechsdk.SpeechRecognitionEventArgs):
            nonlocal last_recognizing_text
            last_recognizing_text = evt.result.text
           # print('RECOGNIZING: {}'.format(evt.result.text))

        def recognized_cb(evt: speechsdk.SpeechRecognitionEventArgs):
            nonlocal last_recognized_text
            last_recognized_text = evt.result.text
        ...
        self.azure_speechrecognizer.recognized.connect(recognized_cb)
        self.azure_speechrecognizer.session_stopped.connect(stop_cb)
        self.azure_speechrecognizer.canceled.connect(canceled_cb)
        ...
        if last_recognized_text or last_recognizing_text:
            final_text = (last_recognized_text + " " + last_recognizing_text).strip()
